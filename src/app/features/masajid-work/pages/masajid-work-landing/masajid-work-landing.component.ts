...
import { MASAJID_WORK_ROUTE_PATH, MW_LANG_STORAGE_KEY } from '../../data/masajid-work.data';
...
export class MasajidWorkLandingComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  onMwClick(event: MouseEvent): void {
    ...
    void this.router.navigate([MASAJID_WORK_ROUTE_PATH], {
      fragment: id,
      replaceUrl: true,
    });
  }
  ...
  private scrollElementIntoView(el: HTMLElement): void {
    ...
    if (reduceMotion) {
      const top =
        ...
        window.scrollY -
        MasajidWorkLandingComponent.SCROLL_HEADER_OFFSET_PX;
